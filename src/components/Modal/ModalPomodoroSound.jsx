...
export default function ModalPomodoroSound({ isOpen, setOpenModal }) {
  ...
  const playSound = (soundFile, soundName) => {
    ...
    const sound = new Howl({
      src: [soundFile],
      html5: true, 
      volume: volume, // Definindo o volume ao criar a instância de som
    });
        ...
        <ul>
          {Object.keys(sounds).map((sound) => (
            ...
            >
              ...
              <span className="text-gray-700 font-medium dark:text-white">{sound}</span>
            </li>
          ))}
        </ul>

        <div className="px-4 pt-4">
          <label className="text-gray-700 dark:text-white">Volume: {Math.round(volume * 100)}%</label>
          <input
            type="range"
            min="0"
            max="1"
            ...
            onChange={(e) => {
              ...
              setVolume(newVolume);
              if (soundInstance) {
                soundInstance.volume(newVolume); // Atualiza o volume da instância de som
              }
            }}
            className="w-full"
          />
