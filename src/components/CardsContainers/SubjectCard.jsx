...
const SubjectCard = ({ banner, nome, atividades, subject }) => {
  ...
  return (
      ...
      <Link to={caminho} onClick={handleSelectSubject}>
        <div className="bg-white dark:bg-zinc-700 border border-gray-200 dark:border-zinc-800 rounded-md hover:scale-102 transition-transform duration-500 cursor-pointer p-4 flex items-center gap-6">
        <span className='bg-gray-100 dark:bg-zinc-800 rounded-full p-3'>
          ...
          <div>
              ...
              <ul className="mt-2 pb-2">
                <li className="mt-2 dark:text-gray-100">Pendentes:</li>
                {atividades &&
                ...
                atividades.length > 0 ? (
                  atividades.map((atividade, index) => (
                    <li key={index} className="flex justify-between text-sm pb-2">
                      {atividade.nome}{' '}
                      <span className="text-sm text-gray-600 dark:text-gray-300">{atividade.data}</span>
                    </li>
                  ))
                ) : (
                  <li className="text-sm dark:text-gray-300 text-gray-600">
